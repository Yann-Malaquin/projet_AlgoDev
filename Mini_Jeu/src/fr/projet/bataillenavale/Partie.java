...
public class Partie {
	...
	private boolean VerifSuperposition(int colonne, int ligne, Bateau b, Joueur j) {
		...
		if (b.getPosition() == false) {
			while (cpt < b.getTaille()) { // Verif superposition verticale
		...
		if (b.getPosition() == true) {
			...
			while (cpt < b.getTaille()) { // Verif superposition horizontale
				if (j.gettBateau().getT()[colonne][ligne + cpt].isModifiable() == true) {
					cpt++;
				}
	...
	public void placerBateau(Joueur joueur) {
		...
		for (Bateau b : Bateau.getTabBateau()) {
			...
			if (b.getPosition() == true) { // Si Position est horizontale
				System.out.println(
						"Votre " + b.getNom() + " de taille : " + b.getTaille() + "  va etre placer horizontalement");
			} else {
				System.out.println(
						"Votre " + b.getNom() + " de taille : " + b.getTaille() + "  va etre placer verticalement");
			}
			...
			if (b.getPosition() == true) { // Si position horizontal
				...
				System.out
						.println("this.VerifSuperposition(y, x, b, joueur)" + this.VerifSuperposition(y, x, b, joueur));
				while (this.VerifSuperposition(y, x, b, joueur) == false) {
					System.out.println(
							"Une des case des deja occupe veuillez entrez des coordonnees qui ne rentre pas en conflit\n");
	...
	public static boolean VerifCoup(Joueur attaquant, Joueur defenseur, int x, int y) {
		...
		if (defenseur.gettBateau().getT()[y][x].getVal() == "X") {
			...
			// grilleJoueur2.retirerElementMap(Integer.valueOf(String.valueOf(x) +
			// String.valueOf(y)));
	 ...
	 * initialiser a un nombre alï¿½atoire pour dï¿½finir le joueur qui va
	 * commencer. On a creer une boucle avec ce compteur qui va etre incrï¿½mentï¿½
	 * ï¿½ l'infini, et une condition d'arret. Quand un des joueur atteint le nombre
	 * de point necessaire pour gagner la partie (le nombre de case ï¿½ couler) la
	 * partie s'arrete. Ensuite le jeu tour ï¿½ tour est gerï¿½ par le compteur qui
	 * est incrï¿½mentï¿½ et qui dï¿½fini le joueur qui joue en fonction de si il
	 * est pair oï¿½ non.
	 */
