...
public class PartiePoker {
	...
	public List<Carte> genererPaquetCarte() {
		...
		String couleur[] = { "Pique", "Tr�fle", "Coeur", "Carreau" }; // tableau regroupant les couleurs
		int i;
	...
	public List<Carte> melangerPaquetCartes(List<Carte> paquetCartes) { // permet de m�langer le paquet de cartes
		...
		while (i > 0) { // tant que l'on a encore des cartes alors on "m�lange" (ajout dans une liste tampon et suppression de la liste initiale)
	...
	public void distribuerCartes(List<Carte> paquetCartes) {
		...
		int init; //d�part du donneur pour distribution
		paquetCartes = this.genererPaquetCarte();
		boolean find=false;
		
		for (int i = 0; i < 2; i++) { // on m�lange le paquet de carte
		...
		for (int tour = 0; tour < 2; tour++) { // pour les tours de distribution
			...
			while (pdonneur < joueurs.size()) { // tant que l'on est pas sup�rieur au nombre de joueur
			...
			pdonneur=init; // on redonne le donneur pour r�d�marrer
		}
	}*/
}
